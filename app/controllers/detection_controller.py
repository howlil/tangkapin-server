...
def create_report(owner_id, array_image, description):
    ...
    try:
        ...
        # Cari POLICE dalam radius 20 km menggunakan Haversine formula
        logger.info("Finding nearby police within 20km radius...")
        nearby_police = db.session.query(
            User.id,
            User.name,
            (6371 * func.acos(
                func.cos(radians(owner_lat)) * func.cos(radians(User.lat.cast(Float))) *
                func.cos(radians(User.lang.cast(Float)) - radians(owner_lang)) +
                func.sin(radians(owner_lat)) * func.sin(radians(User.lat.cast(Float)))
            )).label("distance")
        ).filter(
            User.role == RoleEnum.POLICE,
            User.lat.isnot(None),
    ...
    try:
        ...
        nearby_police = db.session.query(
            ...
            User.name,
            (6371 * func.acos(
                func.cos(radians(owner_lat)) * func.cos(radians(User.lat)) *
                func.cos(radians(User.lang) - radians(owner_lang)) +
                func.sin(radians(owner_lat)) * func.sin(radians(User.lat))
            )).label("distance")
        ).filter(
            User.role == RoleEnum.POLICE,
            User.lat.isnot(None),
